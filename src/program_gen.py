...
class ProblemGenerator:
    ...
    def compose_operations(self, 
                         depth: int, 
                         available_ops: List[DSLOp],
                         used_indices: Set[int],
                         input_size: int,
                         input_type: str = "int") -> Tuple[callable, str, Set[int]]:
    ...
    def generate_problem(self, 
                        ...
                        width: int = 5, 
                        num_examples: int = 6,
                        input_size: int = 5) -> Tuple[ProgramSynthesisProblem, str]:
        ...
        return (ProgramSynthesisProblem(inputs, outputs, available_ops, input_size), target_desc)
    ...
    for depth in range(2, 7):
        for width in range(1, 6):
            for _ in range(16):
                ...
                # print(f"\nGenerated Problem (depth={depth}, width={width}):")
                # print("Available operations:")
                # for op in problem.available_ops:
                #     print(f"  - {op.name}: ({', '.join(['int' for _ in range(len(op.arg_indices))])}) -> {op.output_type}")
        
                # print("\nInput-Output Examples:")
                # for i, (inp, out) in enumerate(zip(problem.inputs[:5], problem.expected_outputs[:5])):
                #     print(f"  Example {i + 1}: {inp} -> {out}")

                # print("What's the output for the following input?")
                # print(f"Input: {problem.inputs[5]} -> ?")
        
                # print(f"\nActual solution: {solution}")
                # print(f"Output: {problem.expected_outputs[5]} -> ?")
                # print("-" * 80)

                problem_desc = f"Available operations:\n"
                for op in problem.available_ops:
                ...
                problem_desc += f"Given the above examples, what is the output for the following input?\n"
...
class ProgramSynthesisSolver:
    ...
    def verify_expression(self, expr: Expression, 
                         examples: List[Tuple[List[int], int]]) -> bool:
        """Check if an expression satisfies all input-output examples"""
        return all(expr.evaluate(inputs) == output 
                  for inputs, output in examples)
    
    def solve(self, examples: List[Tuple[List[int], int]], 
             max_depth: int = 3) -> Optional[Expression]:
