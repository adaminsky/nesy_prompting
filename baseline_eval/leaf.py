...
def leaves_loader(data_root, data_dir, batch_size, n_train, n_test):
  ...
  test_loader = torch.utils.data.DataLoader(test_dataset, collate_fn=LeavesDataset.collate_fn, batch_size=batch_size, shuffle=False)
...
class Scallop_Leaf_Net(nn.Module):
  ...
  def __init__(self, provenance):
    ...
    self.base_ctx = scallopy.ScallopContext(provenance)
    self.base_ctx.import_file("baselines/scallop/leaf/leaves_config.scl")
  ...
  def forward(self, x):
    ...
    return r, margins, textures, shapes
...
class Trainer():
  ...
  def test_epoch(self, epoch):
    ...
    if test_loss < self.min_test_loss and self.phase == "train":
      ...
      torch.save(self.network.margin_net, self.model_root + 'best_margin.pt')
      torch.save(self.network.shape_net, self.model_root + 'best_shape.pt')
      torch.save(self.network.texture_net, self.model_root + 'best_texture.pt')
...
def eval(test_dataset, model_root):
    model = torch.load(model_root + "/best.pt").to(args.device)
    outputs = []
    with torch.no_grad():
      for i, (x, y) in enumerate(test_loader):
        x = x.to(device)
        _, m, s, t = model(x)
        outputs.append((m.cpu(), s.cpu(), t.cpu()))

    with open("scallop_leaf.pkl", "wb") as f:
...
if __name__ == "__main__":
  ...
  model_root = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../leaf"))
